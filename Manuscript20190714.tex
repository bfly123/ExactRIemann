...
\begin{abstract}
  ...
  %{\color{red} In our work,  a four-equation Eulerian hyperbolic governing system is used and the elastic and plastic states are controlled by uniform equations and same EOS. The transformations of elastic and plastic states are  determined by the deviatoric stress spontaneously}. 
...
%them are only considered with  simple constitutive models and simple EOSs.
...
%in this paper, we want to construct an exact Riemann solver for the system of 1D elastic-plastic flows with the Mie-Gr\"uneisen EOS, the hypo-elastic constitutive model and the von Mises' yielding condition. 
  ...
  %As every knows, the Mie-Gr\"uneisen EOS is complex and is used widely in many engineering problems, which makes the energy equation must be included in the system of elastic-plastic flows
%  and so we have to use some special methods to resolve the rarefaction wave.}
  % and so we have to use some special methods to resolve the rarefaction wave
 
   %Otherwise, we analyse the Jacobian matrix and find, in the wave structures of Riemann solvers, there may be three to five waves, including one contact wave and other two to four non-linear waves. These nonlinear waves may be elastic shock waves, elastic rarefaction waves,  plastic shock waves or plastic rarefaction waves. Moreover, elastic waves always runs faster than the followed plastic wave. So there are thirty-six possible cases of the wave structures in the Riemann solver.  Besides, both the changing from elastic to plastic and  the converse changing from plastic to elastic  are considered in the Riemann problem.
...
%The above equations are just for the elastic status of the solid material. If the material is in the plastic state, the above equations can be simplified
%and the jacobian matrix and sonic velocity are different,  correspondingly. We will discuss them separately.
...
Comparing Eq.(\ref{eq:c_e}) with Eq.(\ref{eq:c_p}), we notice that the sonic speed is not continuous between the elastic state and plastic state. As the shear modulus $\mu$ is always positive, the elastic wave runs always faster than the plastic wave.  %This is very important and may cause wrong results if ignoring it.
...
\begin{equation}
  \frac{d\rho}{1} = \frac{d(\rho u)}{u+c_p} = \frac{d(\rho E)}{h+uc_p}.
\end{equation}
Similarly, we can get %  similar  relations  as the left-going wave as
...
\begin{equation} \label{yield1}
  ...
  \begin{aligned}
	...
	& \text{an elastic rarefaction  and a following plastic rarefaction} &  \text{if} \ {s}_{xxL(R)} < \frac{2}{3}Y_{0} \ \text{and} \ \hat{s}_{xxL(R)} \geq \frac{2}{3}Y_{0},\\
	& \text{an elastic shock} & \text{if}  \ \hat{s}_{xxL(R)}> -\frac{2}{3}Y_{0},\\
	& \text{a plastic shock} & \text{if} \ {s}_{xxL(R)} = -\frac{2}{3}Y_{0} \ \text{and} \ \hat{s}_{xxL(R)} \leq -\frac{2}{3}Y_{0},\\
	& \text{an elastic shock and a following plastic shock} &  \text{if} \ {s}_{xxL(R)} > -\frac{2}{3}Y_{0} \ \text{and} \ \hat{s}_{xxL(R)} \leq -\frac{2}{3}Y_{0}.
\end{aligned}\right.
\end{equation}
...
\begin{equation}\label{rerpp}
  \tilde{p}_{L(R)} =
	  p_{L(R)}e^{\frac{\lambda}{\rho_{L(R)}}-\frac{\lambda}{\tilde{\rho}_{L(R)}}} +e^{-\frac{\lambda}{\tilde{\rho}_{L(R)}}}\int_{\rho_{L(R)}}^{\tilde{\rho}_{L(R)}} f_2(x) e^{\frac{\lambda}{x}}dx, %\quad  \rho_{L(R)}\ge \rho \ge \tilde{\rho}_{L(R)},
\end{equation}
where
\begin{equation*}
...
\begin{equation}\label{rerpp2}
 ...
 % \quad   \tilde{\rho}_{L(R)} \ge \rho \ge \rho_{L(R)}^*
\end{equation}
where
\begin{equation*}
  \lambda = \rho_0 \Gamma_0 \quad f_2(\rho) = a_0^2\frac{\partial f}{\partial \eta}- \lambda\frac{s_{xx}(\rho)}{\rho^2}, \ s_{xx}(\rho) =	 -\frac{4}{3}\mu\text{ln}\left(\frac{\rho}{\rho_{L(R)}}\right)+s_{xxL(R)}.
...
\textbf{Remark 2}:  Just like \textbf{Remark 1}, we have to use numerical integral methods introduced in the Appendix A to evaluate the integral terms in (\ref{rerpp2}) and (\ref{rerpp1}). %because we can not get the exact integral values of the two integral terms.


%Now, we can get the states in star regions as
%\begin{equation}
...
%And the state in  the star region is given as
%\begin{equation}
%  p^*_{L(R)} = p(\rho_{L(R)}^*),\quad s_{xxL(R)}^* = s_{xx}(\rho_{L(R)}^*),\quad  u^*_{L(R)} = u(\rho_{L(R)}^*).
%\end{equation}
...
%\end{equation}
  ...
  \item  Evaluate  $\rho^*_{(k+1)}$:
	...
	% By the Newton iteration equation,
...
&	 \text{R: Al,}\quad  \rho = 2785 \text{kg}/\text{m}^3, \quad  u = 0\text{m}/\text{s}, \quad  p = 1.0\text{Pa}, \quad  s_{xx}=-2.0\times 10^8 \text{Pa},\\
 ...
 \right.
\end{equation}
It can be seen that the exact solution matches the numerical result very well in Fig.\ref{fig:case1}.
\begin{figure}[ht]
  \centering
...
&	 \text{R: Al,}\quad  \rho = 2785 \text{kg}/\text{m}^3, \quad  u = 0\text{m}/\text{s}, \quad  p = 1.0\text{Pa}, \quad  s_{xx}=0.0 \text{Pa},\\
 ...
 \right.
\end{equation}
Shown in Fig.\ref{fig:case2}, the exact solution matches the numerical result well generally, except the under-cooling effect performed in the numerical result, but it is not considered in the designing of the exact Riemann solver.
\begin{figure}[ht]
  \centering
...
\begin{equation}
 ...
 \right.
\end{equation}
We can see that the result of the exact solution matches the numerical result very well.
%
...
\begin{equation}
 ...
 \right.
\end{equation}
Results are shown in Fig.\ref{fig:case4},  the result of the exact solver matches the numerical result very well.
...
Obviously, at the beginning, the material is in the negative plastic state (i.e. $s_{xxL(R)}=-\frac{2}{3}{Y}^{0}$). According to the analysis introduced in  Section \ref{sec:relation}, after an expanding process the material will turn back into an elastic state and will yield again into a positive plastic state. Our exact solution in  Fig.\ref{fig:case20} are constitent with the analysis in Section 3.3. Of course, our exact solution matches the numerical solutions very well.
 ...
 %In the previous test, the material is  yielding from the elastic state to the plastic state through the elastic rarefaction wave. In this test, we will show, even if the material is in the negative plastic state at the beginning (i.e. $s_{xxL(R)}=-\frac{2}{3}{Y}^{0}$), after being expanded, the material can be into an elastic state or the positive plastic state, just as we analyze in the introduction. Solving this Rieman problem. This is proved in Fig.\ref{fig:case20} not only by the numerical and theoretical results, the material truly change from a plastic state to an elastic then to a plastic state again. However, if we do not consider the plastic to elastic process there maybe significant wrong in the numerical results as the red line shows.
\begin{figure}[ht]
  \centering
...
%and exact solutions, there is one plastic shock on the left side and both the elastic and plastic shocks exist on the right.
...
\begin{equation}
 ...
 \right.
\end{equation}
Shown in the Fig.\ref{fig:case7}, there is a large jump of density at the material interface and  both the  elastic shock and the plastic shock exist in each side of the interface. Comparing with the numerical result of the scheme with MHLLCEP approximate solver, we can find that our exact Riemann solver can solve the Riemann problem with multi-materials very well.
\begin{figure}[ht]
  \centering
...
\section{Results}
In this paper, we have detailedly analyzed the Riemann problem for one-dimensional multi-material elastic-plastic flows with the  Mie-Gr\"uneisen EOS, hypo-elastic constitutive model and the von Mises' yielding condition and found %some useful results are found through the analysing:
\begin{enumerate}
...
\end{equation}
 ...
 \begin{equation} \label{eq:app}
  p(\rho) \approx
  p_{L(R)}e^{\frac{\lambda}{\rho_{L(R)}}-\frac{\lambda}{\rho}} +e^{-\frac{\lambda}{\rho}} \text{Intg}_1, %\quad  \rho_{L(R)}\ge \rho \ge \tilde{\rho}_{L(R)},
\end{equation}
where
...
\begin{equation}
  c_e(\rho_i) =
  \sqrt{a_0^2 \frac{\partial f}{\partial \eta}\left(\frac{\rho_i}{\rho_0}\right) + \frac{p(\rho_i)}{\rho_i^2}\rho_0\Gamma_0 -\frac{\rho_0}{\rho_i^2}\Gamma_0 s_{xx}(\rho_i) +\frac{4}{3}\frac{\mu}{\rho_i}}. % \quad  \rho_{L(R)} \ge \rho \ge \tilde{\rho}_{L(R)}
\end{equation}
After finishing the evaluation of  $p(\rho_{i})$, we can evaluate $c_e(\rho_i)$ by using the same process as (\ref{eq:app}).

%And we can take an  integration of interval $[\rho_{L(R)}, \rho_i]$ by the same process of Equation (\ref{eq:app}) to solve $p(\rho_i)$.
